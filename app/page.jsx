...
const CATS = [
  ...
  { key: "關鍵報告",tag: "付費報告", title: "關鍵報告",   desc: "把最關鍵的一份判斷寫深寫透——完整推演、結構與風險界線,付費會員專屬。", paid: true },
  { key: "市場觀察",tag: "付費報告\n不定期釋出免費", title: "市場觀察",   desc: "三區塊驅動力矩陣、板塊輪動、主線聚焦與多空劇本。" },
  { key: "美股",    tag: "付費報告\n不定期釋出免費", title: "美股分析",   desc: "REL5 / REL20 / WA 三層篩選,A 級強勢股與輪漲偵測。" },
  { key: "台股",    tag: "付費報告\n不定期釋出免費", title: "台股分析",   desc: "強勢股篩選疊加法人籌碼四型態:吸籌、反轉、換手、警示。" },
  ...
  return (
    <div style={{ marginBottom:28 }}>
      ...
      <div style={{ position:"relative", height:14, borderRadius:8, overflow:"visible",
        ...
        {[25,50,75].map(p=><div key={p} style={{ position:"absolute",top:0,bottom:0,left:`${p}%`,width:1,background:"rgba(255,255,255,.18)" }}/>)}
        <div style={{ position:"absolute",left:`${cycle.big_pct}%`,top:"50%",transform:"translate(-50%,-50%)",zIndex:10 }}>
  ...
  return (
    <div style={{ marginTop:8 }}>
      ...
      <svg viewBox="0 0 900 200" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none"
        ...
        {/* 當前位置指示器(景氣大循環) */}
        <g transform={`translate(${cycle.big_pct/100*900},40.8)`}>
        ...
        {/* 當前位置指示器(製造業週期) */}
        <g transform={`translate(${cycle.mfg_pct/100*900},47.9)`}>
    ...
    fetch(BASE + "/-/reports.json", { cache:"no-store" })
      ...
      .then(data => {
        try {
          ...
          if (db.cycle) next.cycle = db.cycle;
          ...
          if (data.leverage) next.leverage = data.leverage;
  ...
  <div style={{ paddingTop: "1.5rem", borderTop: "0.5px solid var(--border)", textAlign: "center" }}>
    ...
    <a href="https://yoda-wcyc.github.io/game/answer-or-map.html"
      ...
      🎮 玩玩看，你適不適合這裡 →
        ...
        {/* 儀表板 2×2：市場溫度｜總經景氣 · AI泡沫｜全球槓桿 */}
        <section id="dashboards">
          <br />
          <br />
          <div className="eyebrow reveal">Dashboards</div>
            ...
            {CATS.map(c => {
              ...
              return (
                <a key={c.key} className="card cat reveal" href={href} {...extra}>
                  <span className={c.paid ? "tag breathe" : "tag"} style={c.paid ? { display:"inline-block", color:"#080b10", padding:"calc(3px*var(--scale)) calc(10px*var(--scale))", borderRadius:9999, letterSpacing:".04em", fontWeight:700 } : { display:"inline-block", whiteSpace:"pre-line", lineHeight:1.3 }}>{c.tag}</span>
      ...
      <footer>
        <div className="wrap">
          <div className="links">
            ...
            <a href={BASE + "/-/客服.html"}
               style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"5px 14px", background:"none", border:"1px solid var(--gold-lg)", borderRadius:9999, color:"var(--gold-lg)", fontWeight:700, textDecoration:"none", fontSize:"calc(13px*var(--scale))", letterSpacing:".02em" }}>
              ✉ 客服信箱
            </a>
          </div>
          <div className="disc">
            免責聲明:本站所有報告、數據與遊戲內容,均屬財經資訊與投資教育,僅供研究學習參考,不構成投資建議、要約或個別有價證券之買賣推介,亦不構成任何買賣、進出場或部位建議。本服務非證券投資顧問事業,撰稿者未具證券投資分析人員資格,不提供代客操作或獲利保證;文中個股僅為篩選與觀察紀錄,不代表推薦。投資均有風險,決策與盈虧由用戶自行判斷並承擔。數據來源力求準確,但不保證即時與完整。<br/>© 2026 Yoda Research
          </div>
        </div>
      </footer>
    </>
